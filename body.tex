...
\ben{make sure that there is a payoff to concepts we're introducing here \ldots}
\FloatBarrier
\begin{figure}
    \centering
...
\ben{FIXME: rewrite these two paragraphs again \ldots}
...
%Another key piece of metadata is the the ``from" and ``to" compartments of all flows. The ``from" compartments for \emph{per capita} flows are particularly important, as we need to know the population of those compartments in order to calculate the actual flow.
...
\centering
    ...
    \ben{FIXME: move 'label' and 'infectious' to the end, make it clear that this a \emph{set} of other categories to be used as needed}
...
If we assume $\rvec$ is specified in units of people per day and if we use a time step of one day than we can write
\[
