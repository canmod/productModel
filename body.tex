...
To define a compartmental model, we must specify compartments (nodes) and flows (edges). To specify a flow, we record: a from and a to compartment (one will be empty, for a source or sink flow); a rate type (\pc or total) and a rate function that describes how the rate depends on parameters, time and other flows. 
